...
class SQLRetriever:
    ...
    async def _get_schema(self, query: str) -> str:
        ...
        try:
            ...
            if any(k in query_lower for k in ["dispatch", "delivery", "challan", "shipment", "transporter", "vehicle", "driver", "dc", "dc_no", "dc no", "dc number"]):
...
- 4-Step Schema Resolution: Before writing any SQL, strictly resolve:
  ...
  2. Table Selection: Which table physically stores that data? (e.g. product definitions in `product`, machine definitions in `machine`, batch production quantities in `production`, actual invoices in `stock` where `stock_type = 'PI'`, delivery challans in `delivery_challan`, product colors in `product_color`, bin/carton storage locations in `packagings`, measurement units in `unit`, customer orders in `sales_order`).
  3. Column Resolution: Which specific column stores the value? (e.g. DC numbers in `delivery_challan.dc_no`, NOT in `stock.invoice_no`; product color in `product_color.color`, NOT in table `color`; production quantity in `production.qty`, NOT in `actual_production.apq`; product names/codes in `product.product_name`, not in `batch_no` or `unit_name`; storage codes in `packagings.location_code`, not `warehouse`; customer PO in `sales_order.party_po_no`, not `purchase.ref_po_no`).
  4. Relationship & Join Graph: How should the tables be joined? (Follow verified foreign keys directly: `delivery_challan.party_id = party.id`; `production.product_color_id = product_color.id`; `production.product_id = product.id`; `production.machine_id = machine.id`; `stock.party_id = party.id` for invoices).
...
- Warehouse & Packaging Storage Locations: The warehouse master table only stores warehouse facility definitions (id, location_name, warehouse_code). Specific inventory/carton storage locations for products (e.g. 'CHP18080120-INNER') and bin codes (e.g. '762FZNSH') are stored in `packagings.location_code`. To find where a product is stored or what warehouse/bin it is in, ALWAYS join: `packagings pk JOIN warehouse w ON pk.warehouse_id = w.id JOIN product p ON pk.product_id = p.id WHERE (p.product_name LIKE '%<term>%' OR pk.location_code LIKE '%<term>%' OR w.warehouse_code LIKE '%<term>%' OR w.location_name LIKE '%<term>%') AND pk.deleted_at IS NULL AND w.deleted_at IS NULL AND p.deleted_at IS NULL`. Return `p.product_name`, `w.location_name AS warehouse_location`, `pk.location_code`, `pk.carton_no`, `pk.qty`.
...
- Delivery Challan (DC) vs Invoice: A Delivery Challan (DC) and an Invoice are completely separate documents! Actual DC numbers and dates are stored in the `delivery_challan` table: `dc.dc_no` (DC number) and `dc.dc_date` (DC date). The customer/party is linked directly via `delivery_challan.party_id = party.id`. For queries asking for DC numbers, delivery challan list, or DC dates for a party, ALWAYS query: `SELECT p.party_name, dc.dc_no, dc.dc_date FROM delivery_challan dc JOIN party p ON dc.party_id = p.id WHERE ...` NEVER search for DC numbers in `stock.invoice_no` or `stock`! Although `stock` has a `dc_id` column, the actual DC number and date are in `delivery_challan`.
