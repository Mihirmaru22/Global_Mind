...
- 4-Step Schema Resolution: Before writing any SQL, strictly resolve:
  ...
  2. Table Selection: Which table physically stores that data? (e.g. product definitions in `product`, machine definitions in `machine`, batch production quantities in `production`, actual invoices in `stock` where `stock_type = 'PI'`, delivery challans in `delivery_challan`, order due dates in `sales_order`, product colors in `product_color`, bin/carton storage locations in `packagings`, measurement units in `unit`, customer orders in `sales_order`).
  3. Column Resolution: Which specific column stores the value? (e.g. order/delivery due date in `sales_order.so_due_date`; DC numbers in `delivery_challan.dc_no`, NOT in `stock.invoice_no`; product color in `product_color.color`, NOT in table `color`; production quantity in `production.qty`, NOT in `actual_production.apq`; product names/codes in `product.product_name`, not in `batch_no` or `unit_name`; storage codes in `packagings.location_code`, not `warehouse`; customer PO in `sales_order.party_po_no`, not `purchase.ref_po_no`).
  4. Relationship & Join Graph: How should the tables be joined? (Follow verified foreign keys directly: `delivery_challan.sales_order_id = sales_order.id` for due dates; `delivery_challan.party_id = party.id`; `production.product_color_id = product_color.id`; `production.product_id = product.id`; `production.machine_id = machine.id`; `stock.party_id = party.id` for invoices).
